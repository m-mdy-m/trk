...
export class ReportCommand extends BaseCommand {
  ...
  init(): void {
    ...
    const daily = reportCmd.command("daily").description("Today's report").option("-d, --date <YYYY-MM-DD>", "Date to report (default: today)");

    daily.action(this.action((opts: { date?: string }) => this.initDaily(opts)));

    const weekly = reportCmd.command("weekly").description("This week's report").option("-d, --date <YYYY-MM-DD>", "Any date within the target week");

    weekly.action(this.action((opts: { date?: string }) => this.initWeekly(opts)));
    ...
    monthly.action(this.action((opts: { date?: string }) => this.initMonthly(opts)));
  ...
  private printReport(report: Report): void {
    ...
    const formattedRows = rows.map((r) => [r.date, r.label, r.project ?? "—", formatDuration(r.duration), r.note ?? ""]);

    this.printTable(["Date", "Label", "Project", "Duration", "Note"], formattedRows);

    console.log(chalk.bold(`  Total: ${chalk.white(formatDuration(totalSeconds))}\n`));
  }
}
