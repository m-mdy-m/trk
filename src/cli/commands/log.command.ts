...
export class LogCommand extends BaseCommand {
  ...
  init(): void {
    ...
    logCmd.action(this.action((opts: { date?: string }) => this.initList(opts)));

    // `trk log add <label> <hours> [note]`
    const addCmd = logCmd.command("add <label> <hours> [note]").description("Manually add a time log entry");
    addCmd.option("-d, --date <YYYY-MM-DD>", "Override date (defaults to today)");
    addCmd.action(
      this.action((label: string, hoursStr: string, note: string | undefined, opts: { date?: string }) => this.initAdd(label, hoursStr, note, opts)),
    );
  ...
  private initList(opts: { date?: string } = {}) {
    ...
    const rows = entries.map((e) => [e.label, formatDuration(e.duration), e.note ?? "", e.source]);
